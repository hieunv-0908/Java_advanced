package bai04;

public class Main {
    public static void main(String[] args) {
        // ===== PHẦN GỐC CỦA BÀI 1 =====
        TicketPool roomA = new TicketPool("A", 10);
        TicketPool roomB = new TicketPool("B", 10);

        // ===== PHẦN MỚI CỦA BÀI 2 =====
        // Cứ 3 giây thêm 3 vé cho mỗi phòng, lặp 3 lần
        TicketSupplier supplier = new TicketSupplier(roomA, roomB, 3, 3000, 3);

        BookingCounter counter1 = new BookingCounter("Quầy 1", roomA, roomB, supplier);
        BookingCounter counter2 = new BookingCounter("Quầy 2", roomA, roomB, supplier);

        Thread supplierThread = new Thread(supplier);
        Thread t1 = new Thread(counter1);
        Thread t2 = new Thread(counter2);

        // chạy song song
        supplierThread.start();
        t1.start();
        t2.start();

        try {
            supplierThread.join();
            t1.join();
            t2.join();
        } catch (InterruptedException e) {
            e.printStackTrace();
        }

        System.out.println("\n===== TỔNG KẾT =====");
        System.out.println(counter1.getCounterName() + " bán được: " + counter1.getSoldCount() + " vé");
        System.out.println(counter2.getCounterName() + " bán được: " + counter2.getSoldCount() + " vé");
        System.out.println("Vé còn lại phòng A: " + roomA.remainingTickets());
        System.out.println("Vé còn lại phòng B: " + roomB.remainingTickets());
    }
}
